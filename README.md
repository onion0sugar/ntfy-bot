# MSSQL → ntfy (powiadomienia na Androidzie)

Program sprawdza MSSQL i publikuje powiadomienia tekstowe do ntfy.sh lub własnego
serwera ntfy. Na Androidzie zainstaluj aplikację ntfy i zasubskrybuj topic z
`NTFY_TOPIC`. Szczegółowy opis znajduje się w `DOKUMENTACJA_ODTWORZENIA.md`.

Dla trybu wieloużytkownikowego skopiuj `user_mapping.example.json` do
`user_mapping.json`. Każdy login MSSQL musi mieć dwa topiki: `new_order_topic`
i `ready_order_topic`.

> Uwaga: poniższe starsze sekcje README opisujące Zello, FFmpeg i głos dotyczą
> poprzedniej wersji. Aktualny punkt wejścia używa ntfy; konfigurację zacznij od
> `.env.example`.
